...
class Follow8Controller:
    ...
    def run(self, orientation):
        ...
        if abs(orientation - pi / 2) < self.epsilon:
            self.started_8 = True
        if self.started_8 and not self.finishing_8 and abs(orientation) < self.epsilon:
            self.positive_orientation = False
            self.half_8 = True

        if self.half_8 and abs(orientation - 3. * pi / 2.) < self.epsilon:
            self.finishing_8 = True

        if self.finishing_8 and abs(orientation) < self.epsilon:
            self.positive_orientation = True
            self.started_8 = False
            self.half_8 = False
            self.finishing_8 = False
        ...
        if self.debug:
            print("Started 8: {0}; Half 8 {1}; Finishing_8 {2}"
